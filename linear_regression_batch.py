...
def gradientBatchDescent(hypothesis, training_data):
    gradient = [0,0]
    for(first, second) in training_data:
        for i in range(len(hypothesis)):
            gradient[i] += first[i] * (dotProduct(hypothesis , first) - second)
    return [g/len(training_data) for g in gradient]

def trainData(iterations, training_data):
    hypothesis = [0,0]
    for i in range(iterations):
        gradient = gradientBatchDescent(hypothesis, training_data)
...
linReg = trainData(iterations, training_data)
model = linReg
